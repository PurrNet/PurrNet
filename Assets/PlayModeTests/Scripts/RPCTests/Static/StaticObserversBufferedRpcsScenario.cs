...
public class StaticObserversBufferedRpcsScenario : Scenario
{
    ...
    private static int _serverKickoffCount;
    ...
    private async UniTask<ScenarioResult> RunAsServerOnly(ScenarioContext ctx)
    {
        ...
        try
        {
            await UniTaskUtils.WaitWithTimeout(
                () => _serverKickoffCount >= ctx.expectedConnections,
        ...
        {
            return LogAndFail(
                $"Server timed out waiting for client kickoff; got {_serverKickoffCount}/{ctx.expectedConnections}");
        }

        FireBufferedSequence();
    ...
    private async UniTask<ScenarioResult> RunAsHost(ScenarioContext ctx)
    {
        ...
        {
            return LogAndFail(
                $"Host timed out on server-side kickoff count; got {_serverKickoffCount}/{ctx.expectedConnections}");
        }
    ...
    private async UniTask<ScenarioResult> RunAsClient(ScenarioContext ctx)
    {
        ...
        {
            LogFail(failures,$"Buffered static ObserversRpc replay incomplete after reconnect: {ReceiveCountSummary()}");
        }
    ...
    private static void FireBufferedSequence()
    {
        BroadcastBufferedInt(1);
        BroadcastBufferedInt(2);
        BroadcastBufferedInt(FinalInt);

        BroadcastBufferedString("stale-1");
        BroadcastBufferedString("stale-2");
        BroadcastBufferedString(FinalString);

        BroadcastBufferedStruct(new TestPayload { id = 1, label = "stale-1", weight = 1f });
        BroadcastBufferedStruct(new TestPayload { id = 2, label = "stale-2", weight = 2f });
        BroadcastBufferedStruct(new TestPayload { id = FinalStructId, label = FinalStructLabel, weight = FinalStructWeight });

        BroadcastBufferedGeneric<int>(11);
        BroadcastBufferedGeneric<int>(22);
        BroadcastBufferedGeneric<int>(FinalGenericInt);

        BroadcastBufferedDeltaOff(1);
        BroadcastBufferedDeltaOff(7);
        BroadcastBufferedDeltaOff(FinalDeltaOff);

        BroadcastBufferedCompNone(1);
        BroadcastBufferedCompNone(2);
        BroadcastBufferedCompNone(FinalCompNone);

        BroadcastBufferedCompFast(1);
        BroadcastBufferedCompFast(2);
        BroadcastBufferedCompFast(FinalCompFast);

        BroadcastBufferedCompBalanced(1);
        BroadcastBufferedCompBalanced(2);
        BroadcastBufferedCompBalanced(FinalCompBalanced);

        BroadcastBufferedCompBest(1);
        BroadcastBufferedCompBest(2);
        BroadcastBufferedCompBest(FinalCompBest);

        BroadcastBufferedAsyncPackable(new AsyncPayload { seed = 1 });
        BroadcastBufferedAsyncPackable(new AsyncPayload { seed = 2 });
        BroadcastBufferedAsyncPackable(new AsyncPayload { seed = FinalAsyncSeed });
    ...
    {
        Debug.LogError($"[StaticObserversBufferedRpcsScenario] {msg}");
        failures.Add(msg);
    }
    ...
    private static void BroadcastBufferedInt(int payload) { _lastIntReceived = payload; _intReceiveCount++; }

    [ObserversRpc(bufferLast: true)]
    private static void BroadcastBufferedString(string payload) { _lastStringReceived = payload; _stringReceiveCount++; }

    [ObserversRpc(bufferLast: true)]
    private static void BroadcastBufferedStruct(TestPayload payload) { _lastStructReceived = payload; _structReceiveCount++; }

    [ObserversRpc(bufferLast: true)]
    private static void BroadcastBufferedGeneric<T>(T payload)
    {
    ...
    private static void BroadcastBufferedDeltaOff(int payload) { _lastDeltaOffReceived = payload; _deltaOffReceiveCount++; }

    [ObserversRpc(bufferLast: true, compressionLevel: CompressionLevel.None)]
    private static void BroadcastBufferedCompNone(int payload) { _lastCompNoneReceived = payload; _compNoneReceiveCount++; }

    [ObserversRpc(bufferLast: true, compressionLevel: CompressionLevel.Fast)]
    private static void BroadcastBufferedCompFast(int payload) { _lastCompFastReceived = payload; _compFastReceiveCount++; }

    [ObserversRpc(bufferLast: true, compressionLevel: CompressionLevel.Balanced)]
    private static void BroadcastBufferedCompBalanced(int payload) { _lastCompBalancedReceived = payload; _compBalancedReceiveCount++; }

    [ObserversRpc(bufferLast: true, compressionLevel: CompressionLevel.Best)]
    private static void BroadcastBufferedCompBest(int payload) { _lastCompBestReceived = payload; _compBestReceiveCount++; }

    [ObserversRpc(bufferLast: true)]
    private static void BroadcastBufferedAsyncPackable(AsyncPayload p)
    {
